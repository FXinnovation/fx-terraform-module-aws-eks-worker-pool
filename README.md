...
| autoscaling\_group\_desired\_capacity | Desired number of nodes in the worker pool. NOTE: Do not specify it if you plan to do autoscaling. | `number` | n/a | yes |
...
| key\_name | Name of the AWS Key pair to attach to the instances. | `string` | n/a | yes |
...
| spot\_price | The maximum price to use for reserving spot instances. | `string` | n/a | yes |
